	...
	<div class="panel-body" id="hideme">
        <form name="" method="POST" action="" enctype="multipart/form-data">
            ...
            <button type="submit" name="submitlaporan" class="btn btn-primary btn-block">
                <i class="fa fa-television" aria-hidden="true"></i> Rekap Barang Non Aktif
            </button>
        </form>
    </div>
</div>          
...
if(isset($_POST['submitlaporan'])){
    ...
    <h3>
        <?php 
        if ($PERIODE == 'bulan') {
            echo "Daftar Barang Tidak Terdapat Penjualan Bulan: ".namaBulan($A_BULAN)." ".$A_TAHUN; 
        } else {
            echo "Daftar Barang Tidak Terdapat Penjualan Tanggal: ".$TANGGALAWAL." S/D ".$TANGGALAKHIR;
        }
        ?>
    </h3>
    ...
    <?php
        ...
        "SELECT m.namabarang, m.kodebarang, m.satuan, m.namapabrik, SUM(d.stokakhir) AS stokakhir
        FROM masterbarang m 
        LEFT JOIN daftarharga d 
            ON m.kodebarang COLLATE utf8mb4_unicode_ci = d.kodebarang COLLATE utf8mb4_unicode_ci
        WHERE d.kodebarang IS NOT NULL
        AND d.kodebarang COLLATE utf8mb4_unicode_ci NOT IN (
            SELECT DISTINCT 
                x.kodebarang COLLATE utf8mb4_unicode_ci
            FROM detailpenjualan x
            INNER JOIN penjualan y 
                ON x.nopenjualan COLLATE utf8mb4_unicode_ci = y.nopenjualan COLLATE utf8mb4_unicode_ci
            WHERE y.tanggal BETWEEN '".$TANGGALAWAL."' AND '".$TANGGALAKHIR."' 
        ) GROUP BY m.namabarang, m.kodebarang, m.satuan, m.namapabrik
        ORDER BY m.namabarang, m.kodebarang, m.satuan, m.namapabrik");
        
        ?>
        <table class="table table-bordered">
            <thead>
                <tr class="heading-table">
                    <td style='width: 40%' align="center">Nama Barang</td>
                    <td style='width: 15%' align="center">Sat.</td>
                    <td style='width: 30%' align="center">Pabrik</td>
                    <td style='width: 15%' align="center">Stok</td>
                </tr>
            </thead>
            <tbody>
