...
if (!function_exists('getSortUrlPembelian')) {
    function getSortUrlPembelian($column, $currentSortBy, $currentSortOrder, $search, $dateFilter, $rawStartDate, $rawEndDate, $perPage) {
        ...
        return '/pembelian?' . http_build_query(array_filter($params));
    }
}
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="mb-0 me-auto">Daftar Pembelian</h4>
                </div>
                <div class="card-body">
                    <form method="GET" action="/pembelian" class="mb-3" id="filterForm">
                        <div class="row g-2 align-items-end search-filter-card">
                            <div class="col-12 col-lg-3">
                                <input type="text" class="form-control" name="search" placeholder="Cari no pembelian, supplier, barang..." value="<?= htmlspecialchars($search ?? '') ?>">
                            </div>
                            ...
                            <div class="col-12 col-lg-4 d-lg-flex justify-content-lg-end">
                                <div class="row g-2 w-100">
                                    ...
                                    <div class="col-6 col-lg-6">
                                        <a href="/pembelian" class="btn btn-filter btn-outline-secondary w-100">Reset</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <input type="hidden" name="page" value="1">
                        <input type="hidden" name="sort_by" value="<?= htmlspecialchars($sortBy ?? 'tanggalpembelian') ?>">
                        ...
                        <table class="table table-striped table-hover align-middle">
                            <thead class="table-light">
                                <tr>
                                    ...
                                    <th class="th-sortable <?= ($sortBy ?? 'tanggalpembelian') === 'tanggalpembelian' ? (($sortOrder ?? 'DESC') === 'ASC' ? 'sorted-asc' : 'sorted-desc') : '' ?>">
                                        <a href="<?= getSortUrlPembelian('tanggalpembelian', $sortBy ?? 'tanggalpembelian', $sortOrder ?? 'DESC', $search ?? '', $dateFilter ?? 'today', $rawStartDate ?? '', $rawEndDate ?? '', $perPage ?? 10) ?>" class="text-decoration-none text-dark">
                                            Tanggal
                                        </a>
                                    </th>
                                    <th class="th-sortable <?= ($sortBy ?? 'tanggalpembelian') === 'namasupplier' ? (($sortOrder ?? 'DESC') === 'ASC' ? 'sorted-asc' : 'sorted-desc') : '' ?>">
                                        <a href="<?= getSortUrlPembelian('namasupplier', $sortBy ?? 'tanggalpembelian', $sortOrder ?? 'DESC', $search ?? '', $dateFilter ?? 'today', $rawStartDate ?? '', $rawEndDate ?? '', $perPage ?? 10) ?>" class="text-decoration-none text-dark">
                                            Supplier
                                        </a>
                                    </th>
                                    <th>Jumlah</th>
                    ...
                    <nav>
                        <ul class="pagination justify-content-center">
                            <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                                ...
                                ?>
                                <a class="page-link" href="<?= buildPembelianQuery($prevPage, $perPage, $search, $dateFilter, $rawStartDate, $rawEndDate, $sortBy ?? 'tanggalpembelian', $sortOrder ?? 'DESC') ?>">Previous</a>
                            </li>
                            <?php
                            if ($start > 1) {
                                echo '<li class="page-item"><a class="page-link" href="' . buildPembelianQuery(1, $perPage, $search, $dateFilter, $rawStartDate, $rawEndDate, $sortBy ?? 'tanggalpembelian', $sortOrder ?? 'DESC') . '">1</a></li>';
                                if ($start > 2) {
                            ...
                            for ($i = $start; $i <= $end; $i++) {
                                ...
                                echo '<li class="page-item ' . $active . '"><a class="page-link" href="' . buildPembelianQuery($i, $perPage, $search, $dateFilter, $rawStartDate, $rawEndDate, $sortBy ?? 'tanggalpembelian', $sortOrder ?? 'DESC') . '">' . $i . '</a></li>';
                            }
                            if ($end < $totalPages) {
                                if ($end < $totalPages - 1) {
                                    echo '<li class="page-item disabled"><span class="page-link">&hellip;</span></li>';
                                }
                                echo '<li class="page-item"><a class="page-link" href="' . buildPembelianQuery($totalPages, $perPage, $search, $dateFilter, $rawStartDate, $rawEndDate, $sortBy ?? 'tanggalpembelian', $sortOrder ?? 'DESC') . '">' . $totalPages . '</a></li>';
                            }
                            ?>
                            <li class="page-item <?= $page >= $totalPages ? 'disabled' : '' ?>">
                                ...
                                ?>
                                <a class="page-link" href="<?= buildPembelianQuery($nextPage, $perPage, $search, $dateFilter, $rawStartDate, $rawEndDate, $sortBy ?? 'tanggalpembelian', $sortOrder ?? 'DESC') ?>">Next</a>
                            </li>
                        </ul>
                    </nav>
                    <?php endif; ?>
...
<?php
function buildPembelianQuery($page, $perPage, $search, $dateFilter, $rawStartDate, $rawEndDate, $sortBy = null, $sortOrder = null) {
    $params = [
        'page' => max($page, 1),
        'per_page' => $perPage,
        'search' => $search,
    ...
    return '/pembelian?' . http_build_query($params);
}
?>
