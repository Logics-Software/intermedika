...
if (!function_exists('getSortUrlTabelgolongan')) {
    function getSortUrlTabelgolongan($column, $currentSortBy, $currentSortOrder, $search, $perPage, $status) {
        ...
        return '/tabelgolongan?' . $params;
    }
}

if (!function_exists('getSortIconTabelgolongan')) {
    function getSortIconTabelgolongan($column, $currentSortBy, $currentSortOrder) {
        $config = require __DIR__ . '/../../config/app.php';
        $baseUrl = rtrim($config['base_url'], '/');
        if (empty($baseUrl) || $baseUrl === 'http://' || $baseUrl === 'https://') {
            $baseUrl = '/';
            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex align-items-center">
                        <h4 class="mb-0">Daftar Golongan Barang</h4>
                    </div>
                </div>

                <div class="card-body">
                    <div class="row mb-3">
                        <form method="GET" action="/tabelgolongan" id="searchForm">
                            <div class="row g-2 align-items-end">
                                <div class="col-12 col-md-4">
                                    <input type="text" class="form-control" name="search" placeholder="Cari kode atau nama golongan..." value="<?= htmlspecialchars($search) ?>">
                                </div>
                                ...
                                <div class="col-6 col-md-2">
                                    <a href="/tabelgolongan?page=1&per_page=10&status=&sort_by=<?= htmlspecialchars($sortBy) ?>&sort_order=<?= htmlspecialchars($sortOrder) ?>" class="btn btn-filter btn-outline-secondary w-100">Reset</a>
                                </div>
                            </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-hover">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td><?= htmlspecialchars($golongan['kodegolongan']) ?></td>
                                    <td><?= htmlspecialchars($golongan['namagolongan']) ?></td>
                                    <td align="center">
                                        <span class="badge bg-<?= ($golongan['status'] ?? '') === 'aktif' ? 'success' : 'danger' ?>">
                                            <?= htmlspecialchars(ucfirst($golongan['status'] ?? '-')) ?>
                                        </span>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                    ...
                    <nav>
                        <ul class="pagination justify-content-center">
                            <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                                ...
                                ?>
                                <a class="page-link" href="/tabelgolongan<?php echo $buildLink($prevPage); ?>">Previous</a>
                            </li>
                            <?php
                            if ($start > 1) {
                                echo '<li class="page-item"><a class="page-link" href="/tabelgolongan' . $buildLink(1) . '">1</a></li>';
                                if ($start > 2) {
                                    echo '<li class="page-item disabled"><span class="page-link">&hellip;</span></li>';
                                }
                            }
                            for ($i = $start; $i <= $end; $i++) {
                                echo '<li class="page-item ' . ($page == $i ? 'active' : '') . '"><a class="page-link" href="/tabelgolongan' . $buildLink($i) . '">' . $i . '</a></li>';
                            }
                            if ($end < $totalPages) {
                                if ($end < $totalPages - 1) {
                                    echo '<li class="page-item disabled"><span class="page-link">&hellip;</span></li>';
                                }
                                echo '<li class="page-item"><a class="page-link" href="/tabelgolongan' . $buildLink($totalPages) . '">' . $totalPages . '</a></li>';
                            }
                            ?>
                            <li class="page-item <?= $page >= $totalPages ? 'disabled' : '' ?>">
                                ...
                                ?>
                                <a class="page-link" href="/tabelgolongan<?php echo $buildLink($nextPage); ?>">Next</a>
                            </li>
                        </ul>
                    </nav>
                    <?php endif; ?>
